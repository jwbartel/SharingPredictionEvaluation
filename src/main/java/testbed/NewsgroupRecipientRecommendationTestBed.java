...
public class NewsgroupRecipientRecommendationTestBed {
	...
	static {
		...
		// Add data sets
		dataSets.add(new Newsgroups20Dataset("20Newsgroups", new File("data/20 Newsgroups")));
					...
					for (Double wOut : wOuts) {
						...
						for (Double halfLife : halfLives) {
							...
							for (RecipientRecommenderFactory<ComparableAddress> recommenderFactory : recommenderFactories) {
								...
								SingleRecipientRecommendationAcceptanceModeler<ComparableAddress, JavaMailNewsgroupPost> modeler =
										new NewsgroupSingleRecipientRecommendationAcceptanceModeler<>(
												listSize, recommender,
												trainingMessages, testMessages, metrics);
